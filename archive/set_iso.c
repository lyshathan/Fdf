...
/*   Updated: 2024/12/12 11:46:05 by lthan            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	iso_x(t_setup stp, t_point point, int x, int y)
{
	...
	angle = 120;
	dest_x = (x * cos(angle)) + (y * cos(angle + 2)) + ((*(point.z) / stp.h_z) * cos(angle - 2));
	return (dest_x);
}
...
float	iso_y(t_setup stp, t_point point, int x, int y)
{
	...
	angle = 120;
	dest_y = (x * sin(angle)) + (y * sin(angle + 2)) + ((*(point.z) / stp.h_z) * sin(angle - 2));
	return (dest_y);
}
